...
// Region-adaptive timezone abbreviation (PST / CLT / IST / ...) matching the
// displayed time. Falls back to PHP's raw 'T' token if the danger-map module
// (which provides the helper) is unavailable.
$tz_abbr = function ($ts) use ($tz) {
  return function_exists('avalanche_danger_map_tz_abbrev')
    ? avalanche_danger_map_tz_abbrev($ts, $tz)
    : format_date($ts, 'custom', 'T', $tz);
};
...
<article id="node-<?php print $node->nid; ?>" class="am-forecast nac-html-body <?php print $article_classes; ?>"<?php print $article_attributes; ?>>
  ...
  <div id="nac-app">
    ...
    <?php if (!empty($a['expired'])): ?>
      <div class="nac-expired-banner" role="alert">
        ...
        <div class="nac-expired-banner-text">
          ...
          <?php print t('It was issued @date and is no longer current.', array('@date' => format_date($created, 'custom', 'l, F j, Y - g:iA', $tz) . ' ' . $tz_abbr($created))); ?>
    ...
    <div class="nac-header">
      <div class="nac-header-meta"><h6 class="nac-h6"><?php print t('Issued'); ?></h6> <?php print format_date($created, 'custom', 'l, F j, Y - g:iA', $tz) . ' ' . $tz_abbr($created); ?></div>
      <?php if ($expires > $created): ?>
        <div class="nac-header-meta"><h6 class="nac-h6"><?php print t('Expires'); ?></h6> <?php print format_date($expires, 'custom', 'l, F j, Y - g:iA', $tz) . ' ' . $tz_abbr($expires); ?></div>
      <?php endif; ?>
